...
upbitWebSocket.onopen = () => {
    ...
    upbitWebSocket.send(JSON.stringify(subscribeMessage));
};
...
function updateTable() {
    ...
    tradeBody.innerHTML = ""; // 기존 데이터를 초기화
    ...
    console.log("Updated table with data:", recentTrades);
}
...
serverWebSocket.onmessage = (event) => {
    try {
        ...
        if (data.timestamp && data.predicted_price) {
            ...
            // 데이터 업데이트
            recentTrades.unshift({ price: predictedPrice, volume: 1 }); // 가상의 거래량 사용
